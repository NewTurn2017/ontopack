...
cat > "$PACK_DIR/provider.py" <<'PY'
#!/usr/bin/env python3
import json, sys
payload = json.load(sys.stdin)
assert payload["asset_abs_path"]
json.dump({
    "caption": f"Automated vaultworker caption for {payload['note_id']}",
    "tags": ["worker-enriched"],
    "summary": "Generated by the real-test provider command.",
    "provider": "real-test-command",
    "model": "deterministic"
}, sys.stdout)
PY
chmod +x "$PACK_DIR/provider.py"
(cd "$PACK_DIR" && "$PACK_BIN" enrich-pending --provider-command "$PACK_DIR/provider.py" --limit 1 >/tmp/ontopack-real-enrich-pending.out)
...
CLI_WORKER="$(cd "$PACK_DIR" && "$PACK_BIN" search "vaultworker" --mode keyword -k 3)"
